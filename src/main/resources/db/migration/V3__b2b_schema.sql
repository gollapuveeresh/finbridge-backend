-- V3__b2b_schema.sql  ─  B2B Multi-Tenant Tables
...
-- ─────────────────────────────────────────────
-- 2. Organization Users  (multi-role per company)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- 3. Organization Documents  (KYC + service docs)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- 4. Service Requests  (organization ↔ department)
-- ─────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS service_requests (
    id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    request_number      VARCHAR(30) UNIQUE,
    organization_id     UUID NOT NULL REFERENCES organizations(id),
    department_id       VARCHAR(50) NOT NULL,  -- loans | tax | insurance | investment | wealth
    ...
    -- submitted | under_review | documents_required | in_progress | proposal_sent | approved | completed | rejected
...
-- ─────────────────────────────────────────────
-- 5. Organization Proposals  (linked to service request)
-- ─────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS organization_meetings (
    ...
    service_request_id  UUID REFERENCES service_requests(id),
    title               VARCHAR(255) NOT NULL,
    meeting_type        VARCHAR(50)  DEFAULT 'video',  -- video | in_person | phone
